...
> 英国本土垂直合规 / 数据查询站。把 Companies House / HMRC / ONS 的免费官方开放数据，
> 用工程化方式整合成一个**极速、好用、对 SEO 友好的查询站**——查询流量靠 AdSense 变现，
> 批量 / 自动化能力面向 B2B 客户收费（Data Arbitrage / 数据套利）。
>
> 最近更新：2026-08-21 · 代码审查与 README 结构重组、批量鉴权 / CORS 修复。
...
| 章节 | 适合谁 |
|---|---|
| Part 1 · 项目需求（PRD） | 产品 / 商业背景、功能与非功能需求 |
| Part 2 · 项目使用方法 | 跑起来：环境、快速开始、环境变量、页面 / API、认证、变现 |
| Part 3 · 生产部署与日常运维 | 站长操作：上线 / 更新 / 手动事项、目录结构 |
| Part 4 · 常见问题 & 免责声明 | 排查 / FAQ |
...
极速、好用、对 SEO 友好的查询站，把普通查询流量变现，把批量/自动化能力卖给 B2B 客户。**
...
| `ADMIN_EMAILS` | 否 | 管理后台管理员邮箱白名单（逗号分隔） |
| `AUTH_DEMO_MODE` | 否 | `1` 时开放开发演示登录 |
| `LEMONSQUEEZY_*` / `STRIPE_*` | 否 | 收款渠道 |
| `RESEND_API_KEY` / `EMAIL_FROM` | 否 | 申报临期提醒邮件 |
| `CH_SITEMAP_DB` / `CH_LOCAL_DB` / `MEILISEARCH_*` | 否 | 本地公司库 / Meilisearch 补全 / sitemap 数据源 |
...
| `/admin*` | 管理后台（客户 / Key / 套餐 / 设置 / SEO 统计） | 仅管理员邮箱 |
...
| `GET /api/search?q=` | Meilisearch 毫秒级补全（未配置返回 configured:false） |
...
**管理后台**：把管理员邮箱填入 `ADMIN_EMAILS`（逗号分隔），登录后即可访问
`/admin` 及其子页（客户 / Key / 套餐 / 设置 / SEO 统计）。API Key 支持运行时热更新
（`lib/env-config.ts`，保存到 `data/env-config.json`，无需重启）。
...
## 3.3 需要你本人手动操作的事项（按优先级）

### 3.3.1 Companies House API Key（线上已可用，报 401 时再看这里）
...
### 3.3.2 Google AdSense 变现（拿到流量后申请）
...
### 3.3.5 Google Search Console（上线第 1 周做）
...
├── bulk-guide/page.tsx         # 批量 CSV 查询指南
├── pricing|dashboard|account|watchlist/page.tsx
├── admin/*                     # 管理后台（客户/Key/套餐/设置）
...
├── env-config.ts / plan-config.ts / settings.ts # 运行时 Key / 套餐 / 搜索限制
├── search-stats.ts / registry.ts                # 前台搜索统计 / 会员索引
└── schema.ts / format.ts / seed.ts / site.ts / types.ts
scripts/
├── ch-sync/                    # Companies House 全量库同步（快照 + 流式）
└── search/                     # Meilisearch 索引同步
docs/
├── monetization.md             # 定价阶梯 + 收款接入
└── auth.md                     # 用户分层 + Clerk 接入
deploy/                         # VPS 一键部署（deploy.js + ssh2）
...
**Q：管理员登录后看不到 /admin？**
检查 `ADMIN_EMAILS` 是否包含你登录所用邮箱（逗号分隔、忽略大小写）。登录身份来源为
Clerk 或演示会话，邮箱必须与该白名单完全一致。
...
**Q：改 .env.local 里的 Key 后必须重启吗？**
管理后台 `/admin/keys` 保存的 Key 走「运行时覆盖层」（`lib/env-config.ts`）即时生效，无需重启；
但 `REDIS_URL`、Clerk / Stripe 等启动期初始化的参数仍需重启。
